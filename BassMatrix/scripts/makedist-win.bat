...
if %1 == 1 (echo Making BassMatrix Windows DEMO VERSION distribution ...) else (echo Making BassMatrix Windows FULL VERSION distribution ...)
...
msbuild BassMatrix.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild BassMatrix.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\BassMatrix.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\BassMatrix.iss"
goto END-is
